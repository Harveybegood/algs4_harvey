...
public class Ex05_RedBlackBSTWithPrimitiveTypes {
    ...
    public class STdouble{
        private class Node{
            ...
            public Node(double key, double value, boolean color, int numberOfNodes){
                this.key = key;
                this.value = value;
                this.color = color;
                this.numberOfNodes = numberOfNodes;
        ...
        private Node put(Node node, double key, double value){
            if (node == null){return new Node(key, value, RED, 1);}
            if (key < node.key){
                node.left = put(node.left, key, value);
            }
        ...
        public Iterable<Double> keys(double min, double max){
            Queue<Double> queue = new Queue<>();
            keys(root, queue, min, max);
            return queue;
        }
        private void keys(Node node, Queue<Double> queue, double min, double max){
            if (node == null){return;}
            if (min < node.key){
                keys(node.left, queue, min, max);
            }
